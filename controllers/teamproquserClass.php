<?php
class teamproquserClass
{

  public function makeAdmin($user_id,$company_id,$status){
    try{
      $db = getDB();
      $stmt = $db->prepare("UPDATE `teamproq` SET status=:status  WHERE user_id=:user_id AND company_id=:company_id");  
...
$property_edit,$property_delete,$task_read,$task_edit,$task_delete){
  ...
  echo '{"error":{"text":'. $e->getMessage() .'}}'; 
     ...
     public function userLogin($usernameEmail,$password)
     {
          ...
          $stmt = $db->prepare("SELECT user_id FROM teamproq WHERE  (mail=:usernameEmail) AND  password=:hash_password");  
          ...
          if($count)
          {
                $_SESSION['teamproquid']=$data->user_id;
     ...
     $country,$language,$postal_code,$town,$house_no,$street,$status)
     {
          ...
          try{
          $db = getDB();
          $st = $db->prepare("SELECT user_id FROM teamproq WHERE mail=:email");  
          ...
          {
          $stmt = $db->prepare(
            "INSERT INTO teamproq(company_id,salutation,first_name,last_name,company,mail,password,phone,
     ...
     public function userDetails($uid){
        try{
          $db = getDB();
          $stmt = $db->prepare("SELECT * FROM teamproq WHERE user_id=:uid");  
     ...
     public function allUserDetails($cid){
      try{
        $db = getDB();
        $stmt = $db->prepare("SELECT * FROM teamproq WHERE company_id=:cid");  
      ...
      public function profileUpdate($user_id,$FileUpload1){
        try{
          $db = getDB();
          $stmt = $db->prepare("UPDATE teamproq SET logo=:logo WHERE user_id=:user_id");  
          $stmt->bindParam("user_id", $user_id,PDO::PARAM_INT);
