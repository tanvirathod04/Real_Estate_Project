...
this.env.task;if(409==this.env.server_error||bw.dom&&bw.xmlhttp_test()){this.env.blankpage||(this.env.blankpage="about:blank");for(b in this.gui_containers)this.gui_containers[b]=$("#"+this.gui_containers[b]);for(b in this.gui_objects)this.gui_objects[b]=rcube_find_object(this.gui_objects[b]);this.init_buttons();this.is_framed()&&parent.rcmail.unlock_frame();this.enable_command("close","logout","mail","addressbook","settings","save-pref","compose","undo","about","switch-task","menu-open","menu-close",
"menu-save",!0);this.set_button(this.task,"sel");this.env.permaurl&&this.enable_command("permaurl","extwin",!0);switch(this.task){case "mail":this.enable_command("list","checkmail","add-contact","search","reset-search","collapse-folder","import-messages",!0);if(this.gui_objects.messagelist){this.env.widescreen_list_template=[{className:"threads",cells:["threads"]},{className:"subject",cells:["fromto","date","status","subject"]},{className:"flags",cells:["flag","attachment"]}];this.message_list=new rcube_list_widget(this.gui_objects.messagelist,
{multiselect:!0,multiexpand:!0,draggable:!0,keyboard:!0,column_movable:this.env.col_movable,dblclick_time:this.dblclick_time});this.message_list.addEventListener("initrow",function(a){f.init_message_row(a)}).addEventListener("dblclick",function(a){f.msglist_dbl_click(a)}).addEventListener("keypress",function(a){f.msglist_keypress(a)}).addEventListener("select",function(a){f.msglist_select(a)}).addEventListener("dragstart",function(a){f.drag_start(a)}).addEventListener("dragmove",function(a){f.drag_move(a)}).addEventListener("dragend",
function(a){f.drag_end(a)}).addEventListener("expandcollapse",function(a){f.msglist_expand(a)}).addEventListener("column_replace",function(a){f.msglist_set_coltypes(a)}).init();$(this.message_list.thead).on("click","a.sortcol",function(a){return f.command("sort",$(this).attr("rel"),this)});this.enable_command("toggle_status","toggle_flag","sort",!0);this.enable_command("set-listmode",this.env.threads&&!this.is_multifolder_listing());var a=$(this.gui_objects.search_filter).val();a&&"ALL"!=a?this.filter_mailbox(a):
this.command("list");$(this.gui_objects.qsearchbox).val(this.env.search_text).focusin(function(){f.message_list.blur()})}this.set_button_titles();this.env.message_commands="show reply reply-all reply-list move copy delete open mark edit viewsource bounce print load-attachment download-attachment show-headers hide-headers download forward forward-inline forward-attachment change-format".split(" ");if("show"==this.env.action||"preview"==this.env.action){if(this.enable_command(this.env.message_commands,
...
function(a){if("mouseup"==a.type||13==rcube_event.get_keycode(a))return f.command("insert-response",$(this).attr("rel")),$(document.body).trigger("mouseup"),rcube_event.cancel(a)}),$.each(this.buttons["save-response"]||[],function(a,b){$("#"+b.id).mousedown(function(a){return rcube_event.cancel(a)})})),this.init_messageform();else if("bounce"==this.env.action)this.init_messageform_inputs(),this.env.compose_commands=[];else if("get"==this.env.action){this.enable_command("download",!0);this.enable_command("image-scale",
"image-rotate",!!/^image\//.test(this.env.mimetype));if(bw.mz&&"application/pdf"==this.env.mimetype){var b=0;$(this.gui_objects.messagepartframe).on("load",function(){if(b++)try{this.contentWindow.document&&f.enable_command("print",!0)}catch(h){}})}else this.enable_command("print",!0);this.env.is_message&&(this.enable_command("reply","reply-all","edit","viewsource","forward","forward-inline","forward-attachment","bounce",!0),this.env.list_post&&this.enable_command("reply-list",!0));if(this.env.mimetype.startsWith("image/"))$(this.gui_objects.messagepartframe).on("load",
function(){var a=$(this).contents();a.find("img").length&&a.find("head").append('<style type="text/css">img { max-width:100%; max-height:100%; } body { display:flex; align-items:center; justify-content:center; height:100%; margin:0; }</style>')})}else"print"!=this.env.action||!this.env.uid||this.env.is_pgp_content||this.env.pgp_mime_part||this.print_dialog();this.gui_objects.mailboxlist&&(this.env.unread_counts={},this.gui_objects.folderlist=this.gui_objects.mailboxlist,this.http_request("getunread",
{_page:this.env.current_page}));this.gui_objects.contactslist&&(this.contact_list=new rcube_list_widget(this.gui_objects.contactslist,{multiselect:!0,draggable:!1,keyboard:!0}),this.contact_list.addEventListener("initrow",function(a){f.triggerEvent("insertrow",{cid:a.uid,row:a})}).addEventListener("select",function(a){f.compose_recipient_select(a)}).addEventListener("dblclick",function(a){f.compose_add_recipient()}).addEventListener("keypress",function(a){a.key_pressed==a.ENTER_KEY&&(f.compose_add_recipient()||
a.last_selected&&"G"==String(a.last_selected).charAt(0)&&$(a.rows[a.last_selected].obj).find("a").first().click())}).init(),$("#_to,#_cc,#_bcc").focus(function(){f.env.focused_field=this}));this.gui_objects.addressbookslist&&(this.gui_objects.folderlist=this.gui_objects.addressbookslist,this.enable_command("list-addresses",!0));if(this.env.mdn_request&&this.env.uid){a="sendmdn";var d={_uid:this.env.uid,_mbox:this.env.mailbox};confirm(this.get_label("mdnrequest"))||(d._flag="mdnsent",a="mark");this.http_post(a,
d)}this.check_mailvelope(this.env.action);this.is_framed()||this.env.extwin||this.browser_capabilities_check();break;case "addressbook":this.env.address_group_stack=[];this.gui_objects.folderlist&&(this.env.contactfolders=$.extend($.extend({},this.env.address_sources),this.env.contactgroups));this.enable_command("add","import",this.env.writable_source);this.enable_command("list","listgroup","pushgroup","popgroup","listsearch","search","reset-search","advanced-search",!0);this.gui_objects.contactslist&&
...
function(a){f.drag_end(a)}).init(),$(this.gui_objects.qsearchbox).focusin(function(){f.contact_list.blur()}),this.update_group_commands(),this.command("list"));this.gui_objects.savedsearchlist&&(this.savedsearchlist=new rcube_treelist_widget(this.gui_objects.savedsearchlist,{id_prefix:"rcmli",id_encode:this.html_identifier_encode,id_decode:this.html_identifier_decode}),this.savedsearchlist.addEventListener("select",function(a){f.triggerEvent("selectfolder",{folder:a.id,prefix:"rcmli"})}));this.set_page_buttons();
this.env.cid&&(this.enable_command("show","edit","qrcode",!0),this.gui_objects.editform&&$("input.groupmember").change(function(){f.group_member_change(this.checked?"add":"del",f.env.cid,f.env.source,this.value)}));this.gui_objects.editform?(this.enable_command("save",!0),"add"!=this.env.action&&"edit"!=this.env.action&&"search"!=this.env.action||this.init_contact_form()):"print"==this.env.action&&this.print_dialog();break;case "settings":this.enable_command("show","save",!0);"identities"==this.env.action?
this.enable_command("add",2>this.env.identities_level):"edit-identity"==this.env.action||"add-identity"==this.env.action?(this.enable_command("save","edit","toggle-editor",!0),this.enable_command("delete",2>this.env.identities_level),this.editor_init(this.env.editor_config,"rcmfd_signature"),this.check_mailvelope(this.env.action)):"folders"==this.env.action?this.enable_command("subscribe","unsubscribe","create-folder","rename-folder",!0):"edit-folder"==this.env.action&&this.gui_objects.editform?(this.enable_command("save",
"folder-size",!0),parent.rcmail.env.exists=this.env.messagecount,parent.rcmail.enable_command("purge",this.env.messagecount)):"responses"==this.env.action&&this.enable_command("add",!0);this.gui_objects.identitieslist?(this.identity_list=new rcube_list_widget(this.gui_objects.identitieslist,{multiselect:!1,draggable:!1,keyboard:!0}),this.identity_list.addEventListener("select",function(a){f.identity_select(a)}).addEventListener("keypress",function(a){f.list_keypress(a)}).init().focus()):this.gui_objects.sectionslist?
(this.sections_list=new rcube_list_widget(this.gui_objects.sectionslist,{multiselect:!1,draggable:!1,keyboard:!0}),this.sections_list.addEventListener("select",function(a){f.section_select(a)}).init().focus()):this.gui_objects.subscriptionlist?this.init_subscription_list():this.gui_objects.responseslist&&(this.responses_list=new rcube_list_widget(this.gui_objects.responseslist,{multiselect:!1,draggable:!1,keyboard:!0}),this.responses_list.addEventListener("select",function(a){f.response_select(a)}).addEventListener("keypress",
function(a){f.list_keypress(a)}).init().focus());break;case "login":var e=$("#rcmloginuser"),g=$("#rcmlogintz");""==e.val()?e.focus():$("#rcmloginpwd").focus();window.jstz&&(a=jstz.determine())&&(d=a.name());g.val(d?d:(new Date).getStdTimezoneOffset()/-60);$("form").submit(function(){$("[type=submit]",this).prop("disabled",!0);f.clear_messages();f.display_message("","loading")})}this.gui_objects.editform&&$("input,select,textarea",this.gui_objects.editform).not(":hidden").not(":disabled").first().select().focus();
bw.ie&&$("input[type=file]").keydown(function(a){"13"==a.keyCode&&a.preventDefault()});this.loaded=!0;this.env.lastrefresh=new Date;this.pending_message&&this.display_message.apply(this,this.pending_message);this.gui_objects.folderlist&&window.rcube_treelist_widget&&this.gui_objects.folderlist!=this.gui_objects.addressbookslist&&(this.treelist=new rcube_treelist_widget(this.gui_objects.folderlist,{selectable:!0,id_prefix:"rcmli",parent_focus:!0,id_encode:this.html_identifier_encode,id_decode:this.html_identifier_decode,
check_droptarget:function(a){return!a.virtual&&f.check_droptarget(a.id)}}),this.treelist.addEventListener("collapse",function(a){f.folder_collapsed(a)}).addEventListener("expand",function(a){f.folder_collapsed(a)}).addEventListener("beforeselect",function(a){return!f.busy}).addEventListener("select",function(a){f.triggerEvent("selectfolder",{folder:a.id,prefix:"rcmli"});f.mark_all_read_state()}));this.gui_objects.filedrop&&this.env.filedrop&&window.FormData&&($(document.body).on("dragover dragleave drop",
function(a){return f.document_drag_hover(a,"dragover"==a.type)}),$(this.gui_objects.filedrop).addClass("droptarget").on("dragover dragleave",function(a){return f.file_drag_hover(a,"dragover"==a.type)}).get(0).addEventListener("drop",function(a){return f.file_dropped(a)},!1));a=function(a){return f.doc_mouse_up(a)};$(document.body).mouseup(a).keydown(function(a){return f.doc_keypress(a)});rcube_webmail.set_iframe_events({mouseup:a});this.triggerEvent("init",{task:this.task,action:this.env.action});
for(b in this.onloads)if("string"===typeof this.onloads[b])eval(this.onloads[b]);else if("function"===typeof this.onloads[b])this.onloads[b]();$("[data-popup]").each(function(){f.register_menu_button(this,$(this).data("popup"))});this.start_refresh();this.start_keepalive()}else this.goto_url("error","_code=0x199")};this.log=function(a){this.env.devel_mode&&window.console&&console.log&&console.log(a)};this.command=function(a,b,d,e){var g;!d||!d.blur||e&&rcube_event.is_keyboard(e)||d.blur();if(this.busy&&
("reset-search"!=a||"search"!=this.last_command)&&!a.match(/^menu-/))return!1;if(d&&d.href&&0>String(d.href).indexOf("#")&&rcube_event.get_modifier(e))return!0;if(!this.commands[a])return this.is_framed()&&parent.rcmail.command(a,b),!1;if("mail"==this.task&&"compose"==this.env.action&&!this.env.server_error&&"save-pref"!=a&&0>$.inArray(a,this.env.compose_commands)&&!this.compose_skip_unsavedcheck&&!this.env.is_sent&&this.cmp_hash!=this.compose_field_hash())return this.confirm_dialog(this.get_label("notsentwarning"),
"discard",function(){f.remove_compose_data(f.env.compose_id);f.compose_skip_unsavedcheck=!0;f.command(a,b,d,e)}),!1;this.last_command=a;this.command_aborted=!1;this.triggerEvent("actionbefore",{props:b,action:a,originalEvent:e});if(void 0!==(g=this.triggerEvent("before"+a,b||e))){if(!1===g)return!1;b=g}g="function"===typeof this.command_handlers[a]?this.command_handlers[a](b,d,e):"string"===typeof this.command_handlers[a]?window[this.command_handlers[a]](b,d,e):this.command_handler(a,b,d,e);this.command_aborted||
!1!==this.triggerEvent("after"+a,b)||(g=!1);this.triggerEvent("actionafter",{props:b,action:a,aborted:this.command_aborted,ret:g,originalEvent:e});return!1===g||d||!0===this.command_aborted?!1:!0};this.command_handler=function(a,b,d,e){var g;switch(a){case "logout":case "mail":case "addressbook":case "settings":this.switch_task(a);break;case "about":this.redirect("?_task=settings&_action=about",!1);break;case "permaurl":if(d&&d.href&&d.target)return!0;this.env.permaurl&&(parent.location.href=this.env.permaurl);
break;case "extwin":if("compose"==this.env.action){if(a=this.gui_objects.messageform,b=this.open_window(""))this.save_compose_form_local(),this.compose_skip_unsavedcheck=!0,$("[name='_action']",a).val("compose"),a.action=this.url("mail/compose",{_id:this.env.compose_id,_extwin:1}),a.target=b.name,a.submit()}else this.open_window(this.env.permaurl,!0);break;case "change-format":e=this.env.permaurl+"&_format="+b;"preview"==this.env.action&&(e=e.replace(/_action=show/,"_action=preview")+"&_framed=1");
...
a.flagged_children&&!a.expanded;$(a.obj)[b?"addClass":"removeClass"]("flaggedroot")}};this.copy_messages=function(a,b,d){if(a&&"object"===typeof a)a=a.id;else if(!a)return d=this.env.uid?[this.env.uid]:this.message_list.get_selection(),this.folder_selector(b,function(a){f.copy_messages(a,null,d)});a&&a!=this.env.mailbox&&(a=this.selection_post_data({_target_mbox:a,_uid:d}),a._uid&&this.http_post("copy",a,this.display_message("copyingmessage","loading")))};this.move_messages=function(a,b,d){if(a&&
"object"===typeof a)a=a.id;else if(!a)return d=this.env.uid?[this.env.uid]:this.message_list.get_selection(),this.folder_selector(b,function(a){f.move_messages(a,null,d)});a&&(a!=this.env.mailbox||this.is_multifolder_listing())&&(b=!1,a=this.selection_post_data({_target_mbox:a,_uid:d}),a._uid&&("show"==this.env.action&&(b=this.set_busy(!0,"movingmessage")),this.enable_command(this.env.message_commands,!1),this.with_selected_messages("move",a,b),"show"!=this.env.action&&this.show_contentframe(!1)))};
this.delete_messages=function(a){var b=this.message_list,d=this.env.trash_mailbox;if(this.env.flag_for_deletion)return this.mark_message("delete"),!1;d&&this.env.mailbox!=d?this.env.delete_junk&&this.env.junk_mailbox&&this.env.mailbox==this.env.junk_mailbox?this.permanently_remove_messages():b&&b.modkey==SHIFT_KEY||a&&rcube_event.get_modifier(a)==SHIFT_KEY?this.confirm_dialog(this.get_label("deletemessagesconfirm"),"delete",function(){f.permanently_remove_messages()}):this.move_messages(d):this.permanently_remove_messages();
return!0};this.permanently_remove_messages=function(){var a=this.selection_post_data();a._uid&&(this.with_selected_messages("delete",a),this.show_contentframe(!1))};this.with_selected_messages=function(a,b,d,e){var g=0,f="delete"==a||!this.is_multifolder_listing();if(this.message_list){var k,l=[],m=b._uid,q=this.check_display_next();"*"===m?m=this.message_list.get_selection():"string"==typeof m&&(m=m.split(","));var p=0;for(k=m.length;p<k;p++){var u=m[p];if(this.env.threading){g+=this.update_thread(u);
var w=this.message_list.find_root(u);w!=u&&0>$.inArray(w,l)&&l.push(w)}f&&this.message_list.remove_row(u,q&&p==m.length-1)}!q&&f&&this.message_list.clear_selection();p=0;for(k=l.length;p<k;p++)this.add_tree_icons(l[p])}0>g?b._count=-1*g:0<g&&f&&this.delete_excessive_thread_rows();f||(b._refresh=1);d||(d=this.display_message("move"==a?"movingmessage":"deletingmessage","loading"));this.http_post(e||a,b,d)};this.selection_post_data=function(a){"object"!=typeof a&&(a={});a._uid||(a._uid=this.env.uid?
[this.env.uid]:this.message_list.get_selection());a._mbox=this.env.mailbox;a._uid=this.uids_to_list(a._uid);this.env.action&&(a._from=this.env.action);this.env.search_request&&(a._search=this.env.search_request);this.env.display_next&&this.env.next_uid&&(a._next_uid=this.env.next_uid);return a};this.check_display_next=function(){return this.env.display_next&&(this.preview_id||!this.env.contentframe)};this.mark_message=function(a,b){var d=[],e=[],g=this.message_list;b?d[0]=b:this.env.uid?d[0]=this.env.uid:
g&&(d=g.get_selection());if(g){g.focus();var f=0;for(b=d.length;f<b;f++){var k=d[f];("read"==a&&g.rows[k].unread||"unread"==a&&!g.rows[k].unread||"delete"==a&&!g.rows[k].deleted||"undelete"==a&&g.rows[k].deleted||"flagged"==a&&!g.rows[k].flagged||"unflagged"==a&&g.rows[k].flagged)&&e.push(k)}}else e=d;if(e.length||this.select_all_mode)switch(a){case "read":case "unread":this.toggle_read_status(a,e);break;case "delete":case "undelete":this.toggle_delete_status(e);break;case "flagged":case "unflagged":this.toggle_flagged_status(a,
...
"flagged","flagged"==a?!0:!1);if("show"==this.env.action||0<=$.inArray(this.preview_id,b))$(g.document.body)["flagged"==a?"addClass":"removeClass"]("status-flagged");this.http_post("mark",f,k)};this.toggle_delete_status=function(a){var b,d=!0,e=a.length,g=this.message_list?this.message_list.rows:{};if(1==e)return!this.message_list||g[a[0]]&&!g[a[0]].deleted?this.flag_as_deleted(a):this.flag_as_undeleted(a),!0;for(b=0;b<e;b++){var f=a[b];if(g[f]&&!g[f].deleted){d=!1;break}}d?this.flag_as_undeleted(a):
this.flag_as_deleted(a);return!0};this.flag_as_undeleted=function(a){var b,d=a.length,e=this.selection_post_data({_uid:a,_flag:"undelete"}),g=this.display_message("markingmessage","loading");for(b=0;b<d;b++)this.set_message(a[b],"deleted",!1);this.http_post("mark",e,g)};this.flag_as_deleted=function(a){for(var b=[],d=this.selection_post_data({_uid:a,_flag:"delete"}),e=this.display_message("markingmessage","loading"),g=this.message_list,f=g?g.rows:{},k=0,l=this.check_display_next(),m=0,q=a.length;m<
q;m++)uid=a[m],f[uid]&&(f[uid].unread&&(b[b.length]=uid),this.env.skip_deleted?(k+=this.update_thread(uid),g.remove_row(uid,l&&m==g.get_selection(!1).length-1)):this.set_message(uid,"deleted",!0));this.env.skip_deleted&&g&&(l&&g.rowcount||g.clear_selection(),0>k?d._count=-1*k:0<k&&this.delete_excessive_thread_rows());b.length&&(d._ruid=this.uids_to_list(b));this.env.skip_deleted&&this.env.display_next&&this.env.next_uid&&(d._next_uid=this.env.next_uid);this.http_post("mark",d,e)};this.flag_deleted_as_read=
function(a){var b,d=this.message_list?this.message_list.rows:{};"string"==typeof a&&(a=a.split(","));var e=0;for(b=a.length;e<b;e++){var g=a[e];d[g]&&this.set_message(g,"unread",!1)}};this.uids_to_list=function(a){if(this.select_all_mode)return"*";!$.isArray(a)||1!=a.length&&-1!=String(a[0]).indexOf("-")||(a=a.join(","));return a};this.set_button_titles=function(){var a="deletemessage";this.env.flag_for_deletion||!this.env.trash_mailbox||this.env.mailbox==this.env.trash_mailbox||this.env.delete_junk&&
this.env.junk_mailbox&&this.env.mailbox==this.env.junk_mailbox||(a="movemessagetotrash");this.set_alttext("delete",a)};this.init_pagejumper=function(a){$(a).addClass("rcpagejumper").on("focus",function(b){var d,e="";for(d=1;d<=f.env.pagecount;d++)e+="<li>"+d+"</li>";e='<ul class="toolbarmenu menu">'+e+"</ul>";f.pagejump||(f.pagejump=$('<div id="pagejump-selector" class="popupmenu"></div>').appendTo(document.body).on("click","li",function(){f.busy||$(a).val($(this).text()).change()}));f.pagejump.data("count")!=
...
{active:!0});f.env.attachments&&!$.isEmptyObject(f.env.attachments)&&(f.alert_dialog(f.get_label("encryptnoattachments")),$.each(f.env.attachments,function(a,b){f.remove_from_attachment_list(a)}))},function(a){console.error(a);console.log(d)})}};this.mailvelope_submit_messageform=function(a,b){var d=[];$.each(["to","cc","bcc"],function(a,b){for(b=$.trim($('[name="_'+b+'"]').val());b.length&&rcube_check_email(b,!0);)a=RegExp.$2,d.push(a),b=b.substr(b.indexOf(a)+a.length+1).replace(/^\s*,\s*/,"")});
var e=0<d.length;f.mailvelope_keyring.validKeyForAddress(d).then(function(g){var h=[];$.each(g,function(a,b){!1===b&&(e=!1,h.push(a))});if(!e&&h.length)return f.simple_dialog(f.get_label("nopubkeyfor").replace("$email",h.join(", "))+"<p>"+f.get_label("searchpubkeyservers")+"</p>","encryptedsendialog",function(){f.mailvelope_search_pubkeys(h,function(){return!0})},{button:"search"}),!1;if(!e)return d.length||f.alert_dialog(f.get_label("norecipientwarning"),function(){$("[name='_to']").focus()}),!1;
var k=[],l=f.env.identities[$("[name='_from'] option:selected").val()];$.each(f.env.identities,function(a,b){k.push(b.email)});f.mailvelope_keyring.validKeyForAddress(k).then(function(e){valid_sender=null;$.each(e,function(a,b){if(!1!==b&&(valid_sender=a,valid_sender==l))return!1});if(!valid_sender&&!confirm(f.get_label("nopubkeyforsender")))return!1;d.push(valid_sender);f.mailvelope_editor.encrypt(d).then(function(d){var e=f.gui_objects.messageform,g=$("[name='_pgpmime']",e),h=f.set_busy(!0,a||b?
"savingmessage":"sendingmessage");e.target=f.get_save_target();e._draft.value=a?"1":"";e.action=f.add_url(e.action,"_unlock",h);e.action=f.add_url(e.action,"_framed",1);b&&(e.action=f.add_url(e.action,"_saveonly",1));g.length||(g=$('<input type="hidden" name="_pgpmime">').appendTo(e));g.val(d);e.submit()},function(a){console.log(a)})},function(a){console.error(a)})},function(a){console.error(a)});return!1};this.mailvelope_display_container=function(a,b,d,e){var g=function(b){$(a+" > iframe").remove();
...
this.contact_list.get_selection(),d=this.check_display_next();for(a=0;a<b.length;a++)id=b[a],this.contact_list.remove_row(id,d&&a==b.length-1);d||this.contact_list.clear_selection()}};this.insert_contact_group=function(a){a.type="group";var b="G"+a.source+a.id,d=$("<a>").attr({href:"#",rel:a.source+":"+a.id}).click(function(){return f.command("listgroup",a,this)}).html(a.name);this.env.contactfolders[b]=this.env.contactgroups[b]=a;this.treelist.insert({id:b,html:d,classes:["contactgroup"]},a.source,
...
g.id=a.newid;g.type="group";d.id=e;d.html=$("<a>").attr({href:"#",rel:a.source+":"+a.newid}).click(function(){return f.command("listgroup",g,this)}).html(a.name)}else $(this.treelist.get_item(b)).children().first().html(a.name),this.env.contactfolders[b].name=this.env.contactgroups[b].name=a.name,a.source==this.env.source&&a.id==this.env.group&&this.set_group_prop(a);this.treelist.update(b,d,!0);this.destroy_entity_selector("addressbook-selector");this.destroy_entity_selector("contactgroup-selector");
...
t=t.show().get(0);t.scrollIntoView&&t.scrollIntoView(!1);return t};this.replace_folder_row=function(a,b,d,e,g,h){if(!this.gui_objects.subscriptionlist)return this.is_framed()?window.parent.rcmail.replace_folder_row(a,b,d,e,g,h):!1;this.subscription_list.is_search()&&(this.subscription_select(),this.subscription_list.reset_search());var k={},l=this.subscription_list.get_item(a,!0),m=$(l).parent(),q=a.length,p=this.env.subscriptionrows[a][0].length,u=$('input[name="_subscribed[]"]',l).first().prop("checked");
a==b?$(l).attr("class",h||""):($("li",l).each(function(){var a=f.folder_id2name(this.id),e=f.env.subscriptionrows[a],g=b+a.slice(q);this.id="rcmli"+f.html_identifier_encode(g);$('input[name="_subscribed[]"]',this).first().val(g);e[0]=d+e[0].slice(p);k[g]=e;delete f.env.subscriptionrows[a]}),l=$(l).detach(),delete this.env.subscriptionrows[a],m.get(0)==this.gui_objects.subscriptionlist||$("li",m).length||$("ul,div.treetoggle",m.parent()).remove(),this.add_folder_row(b,d,e,g,u,h,l,k))};this.remove_folder_row=
