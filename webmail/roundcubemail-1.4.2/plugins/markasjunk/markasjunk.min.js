...
0<a.get_selection(!1).length)});rcmail.markasjunk_toggle_button()}),rcmail.addEventListener("listupdate",function(){rcmail.markasjunk_toggle_button()}),rcmail.addEventListener("beforemoveto",function(a){a&&"object"===typeof a&&(a=a.id);var c=null;rcmail.env.markasjunk_move_spam&&a&&a==rcmail.env.markasjunk_spam_mailbox&&a!=rcmail.env.mailbox?c=!0:rcmail.env.markasjunk_move_ham&&a&&a==rcmail.env.markasjunk_ham_mailbox&&rcmail.env.mailbox==rcmail.env.markasjunk_spam_mailbox&&(c=!1);if(null!==c)return rcmail.markasjunk_mark(c),
!1}));
